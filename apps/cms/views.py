...
from .forms import EditNewsCategoryForm,WriteNewsForm,AddBannerForm,EditBannerForm,EditNewsForm,EditTeacherForm
import os
import qiniu
...
def teacher(request):
    ...
    return render(request,'cms/teacher.html',context=context)

@require_POST
def add_teacher(request):
    name = request.POST.get('name')
    Teacher.objects.create(username=name)
...
def delete_teacher(request):
    ...
    try:
        ...
        return restful.params_error(message='老师不存在')
...
def edit_teacher(request):
        ...
        name = request.POST.get('name')
        try:
            Teacher.objects.filter(pk=pk).update(username=name)
            return restful.ok()
        except:
            return restful.params_error(message='老师不存在')
